...
class SparseWindowAttention(nn.Module):
    def __init__(self, dim, n_head, window_size, pool_size=(4,4), qkv_bias=True, attn_drop=0., proj_drop=0., 
        ...
        self.q_prior = nn.Linear(128, dim, qkv_bias)
        self.k_prior = nn.Linear(256, dim, qkv_bias)
        self.v_prior = nn.Linear(256, dim, qkv_bias)
    ...
    def inint_projection(self):
        ...
        nn.init.constant_(self.q_prior.weight.data, 0)
        nn.init.constant_(self.k_prior.weight.data, 0)
        nn.init.constant_(self.v_prior.weight.data, 0)
        nn.init.constant_(self.q_prior.bias.data, 0)
        nn.init.constant_(self.k_prior.bias.data, 0)
        nn.init.constant_(self.v_prior.bias.data, 0)


        nn.init.normal_(self.proj.weight.data, 0.0, 0.015)
        nn.init.constant_(self.proj.bias.data, 0.0)
    
    def forward(self, x, qmask, kvmask, T_ind=None, prior=None):
        ...
        q_map = self.q_prior(q_prior)
        k_map = self.k_prior(kv_prior)
        v_map = self.v_prior(kv_prior)

        q = self.query(x + q_map)
        k = self.key(x + k_map)
        v = self.value(x + v_map)
        ...
        att_t = (win_q @ win_k.transpose(-2, -1)) * (1.0 / math.sqrt(win_q.size(-1)))
        att_t = F.softmax(att_t, dim=-1)
        att_t = self.attn_drop(att_t)
        y_t = att_t @ win_v 
        y_t = y_t.view(-1, self.n_head, t_token, w_h*w_w, c_head)
        y_t = y_t.permute(0,2,1,3,4).reshape(-1, self.n_head, w_h*w_w, c_head)
        out = y_t.view(b, n_wh, n_ww, t, self.n_head,  w_h, w_w, c_head)
...
class TemporalSparseTransformer(nn.Module):
    ...
    def forward(self, x, fold_x_size, qmask, kvmask, T_ind=None , q_attn=None):
        """
        Args:
        ...
        att_x = self.attention(x, qmask, kvmask, T_ind, q_attn)
...
class TemporalSparseTransformerBlock(nn.Module):
    ...
    def forward(self, x, fold_x_size, qmasks, kvmasks, t_dilation=2, q_attn=None):
        """
        Args:
            x: image tokens, shape [B T H W C]
            fold_x_size: fold feature size, shape [60 108]
        ...
        for i in range(0, self.depths):
            x = self.transformer[i](x, fold_x_size, qmasks, kvmasks, T_ind[i], q_attn)
        return x
