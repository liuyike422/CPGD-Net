    ...
    for d in slice(-window_size[0]), slice(-window_size[0], -shift_size[0]), slice(-shift_size[0], None):
        for h in slice(-window_size[1]), slice(-window_size[1], -shift_size[1]), slice(-shift_size[1], None):
            for w in slice(-window_size[2]), slice(-window_size[2], -shift_size[2]), slice(-shift_size[2], None):
                img_mask[:, d, h, w, :] = cnt
                cnt += 1
    mask_windows = window_partition(img_mask, window_size)  # nW, ws[0]*ws[1]*ws[2], 1 #384, 128(2,8,8), 1
    mask_windows = mask_windows.squeeze(-1)  # nW, ws[0]*ws[1]*ws[2]
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2) #(384,1,128) - (384,128,1)
    attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))#(384,128,128)
...
class WindowAttention_resi(nn.Module):
    """ Window based multi-head self attention.
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None):
        ...
        self.proj_y = nn.Linear(1,192)
        self.qkv_y = nn.Linear(dim, dim*3, bias=qkv_bias)

    def forward(self, x, y, resi_type, mask=None):
        """ Forward function.
        ...
        B_, N, C = x.shape
        qkv = self.qkv_self(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q_x, k_x, v_x = qkv[0], qkv[1], qkv[2]  # B_, nH, N, C

        # attention mask
        y = self.proj_y(y)
        qkv_y = self.qkv_y(y).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q_y, k_y, v_y = qkv_y[0], qkv_y[1], qkv_y[2]  # B_, nH, N, C

        q = torch.cat((q_x,q_y), dim = 3)
        k = torch.cat((k_x,k_y), dim = 3)
        v = torch.cat((v_x,v_y), dim = 3)
            
        x_out = self.attention(q, k, v, mask, (B_, N, C*2))
...
class STL_resi(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention_resi(dim, window_size=self.window_size, num_heads=num_heads, qkv_bias=qkv_bias,
                                    qk_scale=qk_scale)
        self.norm2 = norm_layer(dim)
        self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer)

    def forward_part1(self, in_fea, resi_type, mask_matrix):

        # x = in_fea
        x = in_fea[:,:,:,:,:192]
        y = in_fea[:,:,:,:,192].unsqueeze(4)
        B, D, H, W, C = x.shape
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)
        ...
        pad_r = (window_size[2] - W % window_size[2]) % window_size[2]
        x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1), mode='constant')
        y = F.pad(y, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1), mode='constant')
        ...
        # attention / shifted attention
        attn_windows = self.attn(x_windows, y_windows, resi_type, mask=attn_mask)  # B*nW, Wd*Wh*Ww, C
    ...
    def forward(self, in_fea, resi_type, mask_matrix):
        """ Forward function.
        ...
        if self.use_checkpoint_attn:
            ...
            x = x + self.forward_part1(in_fea, resi_type, mask_matrix)
...
class STG(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, x.device)#(384,128,128)
...
class STG_resi(nn.Module):
    ...
    def forward(self, in_fea):
        ...
        Wp = int(np.ceil(W / window_size[2])) * window_size[2]
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, x.device)#(384,128,128)
        # residual生成attention mask

        in_feat = torch.cat((x,y), dim = 4)
        i = 0
        for blk in self.blocks:
            if i % 2 == 0:
                resi_type = 'kv'
                ...
                resi_type = 'q'
            out_feat = blk(in_feat, resi_type, attn_mask)
            i += 1
        x = out_feat[:,:,:,:,:192]
...
class STB_resi(nn.Module):
    ...
    def __init__(self, **kwargs):
        super(STB_resi, self).__init__()
        self.input_resolution = kwargs['input_resolution']

        self.residual_group = STG_resi(**kwargs)
        self.linear = nn.Linear(kwargs['dim'], kwargs['dim'])
    ...
    def forward(self, in_fea):
        ...
        x = x + self.linear(self.residual_group(in_fea).transpose(1, 4)).transpose(1, 4)

        out_fea = torch.cat((x,y), dim=1)
        return out_fea

class ST_Resi(nn.Module):
    ...
    def __init__(self, num_blocks=2, **kwargs):
        ...
        self.resi_ds += [
                            ...
                            nn.ReLU(inplace=True),
                            nn.Conv2d(8, 1, kernel_size=3, stride=2, padding=1, bias=False),
                            nn.ReLU(inplace=True),     
                        ]
        self.resi_ds = nn.Sequential(*self.resi_ds)    
        
        # # RSTB blocks
        kwargs['use_checkpoint_attn'] = kwargs.pop('use_checkpoint_attn')[0]
        kwargs['use_checkpoint_ffn'] = kwargs.pop('use_checkpoint_ffn')[0]
        
        self.STBlock = make_layer(STB_resi, num_blocks, **kwargs)

        self.norm = nn.LayerNorm(kwargs['dim'])

    def forward(self, x, y_full):
        """
        Forward function for RSTBWithInputConv.

        Args:
            feat (Tensor): Input feature with shape (n, t, in_channels, h, w) 
            resi (Tensor): Input residual with shape(n, t, 1, h, w) full size         

        Returns:
            Tensor: Output feature with shape (n, t, out_channels, h, w)
        """
        b,t,_,h,w = y_full.shape
        y = self.resi_ds(y_full.reshape(b*t,_,h,w))
        y = y.reshape(b,t,1,h//4,w//4)

        x = Rearrange('n d c h w -> n c d h w ')(x)
        y = Rearrange('n d c h w -> n c d h w ', c=1)(y)
        in_fea = torch.cat((x,y), dim=1)#shape n c d h w (1,193,12,64,64)

        out_fea = self.STBlock(in_fea)
        x = out_fea[:,:192,:,:,:]

        #org
        x = Rearrange('n c d h w -> n d h w c')(x)
        x = self.norm(x)
        x = Rearrange('n d h w c -> n d c h w')(x)
...
def get_resi_map(resis, flow_b):
    b,t,c,h,w = resis.shape
    resi_min,_ = torch.min(resis.reshape(b,-1),dim=-1)
    resi_max,_ = torch.max(resis.reshape(b,-1),dim=-1)
    resi_map = (resis - resi_min.reshape(b,1,1,1,1))/(resi_max-resi_min+0.0025).reshape(b,1,1,1,1) + 0.002
    return resi_map
...
class BSST(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()#固定transformer权重，训练前段 12.19
        ...
        # for param in self.concat.parameters():
        #     param.requires_grad = False
        # for param in self.act.parameters():
        #     param.requires_grad = False
        # for param in self.down01.parameters():
        #     param.requires_grad = False
        ...
        # for param in self.encoder_level1.parameters():
        #     param.requires_grad = False
        # for param in self.encoder_level1_1.parameters():
        #     param.requires_grad = False
        # for param in self.encoder_level2.parameters():
        #     param.requires_grad = False
        # for param in self.encoder_level2_1.parameters():
        #     param.requires_grad = False
        
        self.down12 = nn.Sequential(nn.Conv2d(n_feat, 192, 3, 2, 1, bias=False), act)
        #
        # for param in self.down12.parameters():
        #     param.requires_grad = False
        ...
        #固定transformer权重，训练前段 12.19
        # for param in self.decoder_level1.parameters():
        #     param.requires_grad = False
        # for param in self.decoder_level1_1.parameters():
        #     param.requires_grad = False
        # for param in self.decoder_level1_2.parameters():
        #     param.requires_grad = False
        # for param in self.decoder_level2.parameters():
        #     param.requires_grad = False
        # for param in self.decoder_level2_1.parameters():
        #     param.requires_grad = False
        # for param in self.decoder_level2_2.parameters():
        #     param.requires_grad = False
        # for param in self.skip_attn1.parameters():
        #     param.requires_grad = False
        # for param in self.upsample0.parameters():
        #     param.requires_grad = False
        # for param in self.skip_conv.parameters():
        #     param.requires_grad = False
        # for param in self.out_conv.parameters():
        #     param.requires_grad = False
        # for param in self.last_conv.parameters():
        #     param.requires_grad = False
        # for param in self.conv_hr0.parameters():
        #     param.requires_grad = False
        ...
        # for i, module in enumerate(modules):
        #     for param in self.deform_align[module].parameters():
        #         param.requires_grad = False
        ...
        # self.cabattn = nn.Sequential(
        #     CAB2(192, 5, 4, bias=False, act=self.act), 
        #     CAB1(192, 5, 4, bias=False, act=self.act)
        # )
        
        # self.upsampleLP = PixelShufflePack(4, 16, 2, upsample_kernel=3)
        # self.softsplitLP = SoftSplit(16, 256, (5, 5), (2, 2), (2, 2),gain=0.01)
        # self.softsplitResi = SoftSplit(1, 128, (5, 5), (2, 2), (2, 2),gain=0.01)
    ...
    def init_SparseTransformer(self):
        ...
        # #固定transformer权重，训练前段 12.19
        # for param in self.transsqattn.parameters():
        #     param.requires_grad = False
        # for param in self.softcomp.parameters():
        #     param.requires_grad = False
        # for param in self.cabattn.parameters():
        #     param.requires_grad = False
    ...
    def propagate(self, feats, flows, flows_check, blur_motion_map, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            # feat_skip = [feats['spatial'][idx]]
            # feat_skip = torch.cat(feat_skip, dim=1)
            # f_mask = self.f_mask(torch.cat((blur_motion_map[:,i,:,:,:], feat_skip, feat_prop), dim=1))
            # f_mask = torch.sigmoid(f_mask)
            # feat_prop = f_mask * feat_skip + (1-f_mask) * feat_prop
    ...
    def forward(self, lqs, flows_forwards_gt, flows_backwards_gt, resis, lp=None):
        ...
        resis = torch.abs(resis)
        resis = F.interpolate(resis.reshape(n*t,1,h,w), scale_factor=0.25, mode='bilinear').view(n,t,1,h//4,w//4)
        ...
        resi_map = get_resi_map(resis, flows_backwards_gt)
        ...
        for iter_ in [1, 2]:
            for direction in ['backward', 'forward']:
                ...
                feats[module_name] = []
                feats = self.propagate(feats, flows, flows_check, resi_map, module_name)
        ...
        fusion_in = torch.cat([feats[k] for k in feats], dim=2)
        fu_feats = self.reconstruction(fusion_in)
        
        ###########################################################
        # input_feats = fu_feats
        ...
        ############################################################################################
        trans_feats = torch.zeros_like(fu_feats)
        trans_feats = self.cabattn(torch.cat([fu_feats,trans_feats],2)[0]).unsqueeze(0)
