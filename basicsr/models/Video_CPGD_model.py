...
from DiffBIR_main.diffbir.utils.common import wavelet_reconstruction
...
class ModelBSST(BaseModel):
    ...
    def get_bi_flows(self,lq, mv_f=None, mv_b=None):
        ...
        with torch.no_grad():
            ...
            if mv_b is not None:
            #     # mv_f = mv_f.reshape(b*(t-1),2,h//4,w//4)
                mv_b = mv_b.reshape(b*(t-1),2,h//4,w//4)
            #     # mv_f = F.interpolate(mv_f, scale_factor=0.25, mode='bilinear')*0.25
                mv_b = F.interpolate(mv_b, scale_factor=0.25, mode='bilinear')*0.25
                iter = 4

            
            # flows_forwards_pred = self.fix_raft(lq2,lq1,iters = iter, flow_init = mv_f).detach()
            flows_backwards_pred = self.fix_raft(lq1,lq2, iters = iter, flow_init = mv_b).detach()

            # flows_forwards_pred = self.fix_raft(lq2,lq1,iters = iter).detach()
            # flows_backwards_pred = self.fix_raft(lq1,lq2, iters = iter).detach()
    ...
    def generation_tile(self, current_iter, seq_num, folder):
        ...
        with torch.no_grad():
            ...
            for i in t_list:
                ...
                cp_i = cp[i:i+diff_bs,:,:,:]
                cp_rs = F.interpolate(cp_i, scale_factor=0.25, mode='bilinear', align_corners=True).reshape(b,diff_bs//b,3,h//4,w//4)
    ...
    def generation_tile_wo_rs(self, current_iter, seq_num, folder):
        ...
        with torch.no_grad():
            ...
            for i in t_list:
                ...
                cp_i = cp[i:i+diff_bs,:,:,:]
                cp_rs = F.interpolate(cp_i, scale_factor=0.125, mode='bilinear', align_corners=True).reshape(b,diff_bs//b,3,h//8,w//8)
                img = self.output.reshape(b*t,3,h,w).float()[i:i+diff_bs,:,:,:]
                img = torch.clip(img, min=0, max=1)
                cond_img = (img * 2 - 1).to(self.device)
                # cond_img = F.interpolate(cond_img, scale_factor=2, mode='bilinear', align_corners=True)
