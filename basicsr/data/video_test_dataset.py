...
from basicsr.utils.utils_tools import load_mv_seq, load_resi_seq
...
class VideoDeblurTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['name'] == 'BSD':
            ...
            self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
            self.mv_root = opt['dataroot_mv']
            self.resi_root = opt['dataroot_resi']
        self.data_info = {'lq_path': [], 'gt_path': [], 'mv_path': [], 'resi_path': [], 'folder': [], 'idx': [], 'border': [],}
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        assert self.io_backend_opt['type'] != 'lmdb', 'No need to use lmdb during validation/test.'
        ...
        self.mv_f, self.mv_b = {}, {}
        self.resi = {}
        subfolders_lq, subfolders_gt, subfolders_mv, subfolders_resi = [],[],[],[]
        if opt['name'] == 'BSD':
            for dirs in os.listdir(self.data_root):
                subfolders_lq.append(osp.join(self.data_root, dirs, 'Blur/RGB'))
                subfolders_gt.append(osp.join(self.data_root, dirs, 'Sharp/RGB'))
                subfolders_mv.append(osp.join(self.data_root, dirs, 'Blur/MVs'))
                subfolders_resi.append(osp.join(self.data_root, dirs, 'Blur/resi'))
            ...
            if 'meta_info_file' in opt:
                ...
                subfolders_resi = sorted(glob.glob(osp.join(self.resi_root, '*')))


        if opt['name'].lower() in ['vid4', 'reds4', 'redsofficial','dvd','gopro', 'bsd']:
            for subfolder_lq, subfolder_gt, subfolders_mv, subfolders_resi in zip(subfolders_lq, subfolders_gt, subfolders_mv, subfolders_resi):
                # get frame list for lq and gt
                if opt['name'] == 'BSD':
                    subfolder_name = subfolder_lq.split('/')[-3]
                else:
                    subfolder_name = osp.basename(subfolder_lq)
                img_paths_lq = sorted(list(scandir(subfolder_lq, full_path=True)))
                img_paths_gt = sorted(list(scandir(subfolder_gt, full_path=True)))
                mv_paths = sorted(list(scandir(subfolders_mv, full_path=True)))
                resi_paths = sorted(list(scandir(subfolders_resi, full_path=True)))
                ...
                self.data_info['lq_path'].extend(img_paths_lq)
                self.data_info['gt_path'].extend(img_paths_gt)
                self.data_info['mv_path'].extend(mv_paths)
                self.data_info['resi_path'].extend(resi_paths)
                self.data_info['folder'].extend([subfolder_name] * max_idx)
                for i in range(max_idx):
                    self.data_info['idx'].append(f'{i}/{max_idx}')
                border_l = [0] * max_idx
                ...
                if self.cache_data:
                    ...
                    self.imgs_lq[subfolder_name] = img_paths_lq
                    self.imgs_gt[subfolder_name] = img_paths_gt
                    self.mv_f[subfolder_name] = mv_paths
                    self.resi[subfolder_name] = resi_paths
        else:
            raise ValueError(f'Non-supported video test dataset: {type(opt["name"])}')
...
class VideoRecurrentTestDatasetlocal(VideoDeblurTestDataset):
            ...
            for folder in self.folders:
                ...
                if d < num_frame_testing:
                    ...
                    for d_index in d_idx_list:
                        ...
                        selected_mv_index = [
                            i for i in range(d_index,d_index+num_frame_testing-1)
                        ]
                        self.splite_seqs_index[f"{folder}_{d_index}"] = {
                            "seq_index" : selected_index,
                            "seq_mv_idx" : selected_mv_index,
    ...
    def __getitem__(self, index):
        ...
        selected_index,selected_mv_index,folder = self.splite_seqs_index[seq_name]["seq_index"],self.splite_seqs_index[seq_name]["seq_mv_idx"],self.splite_seqs_index[seq_name]["seq_from_folder"]
        lq_paths = [ self.imgs_lq[folder][p] for p in selected_index]
        gt_paths = [ self.imgs_gt[folder][p] for p in selected_index]
        mv_paths = [ self.mv_f[folder][p] for p in selected_mv_index]
        resi_paths = [ self.resi[folder][p] for p in selected_mv_index]

        imgs_lq = read_img_seq(lq_paths)
        imgs_gt = read_img_seq(gt_paths)
        mv_f = load_mv_seq(mv_paths)
        resi = load_resi_seq(resi_paths)
