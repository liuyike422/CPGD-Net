...
class InferenceLoop:
    ...
    def load_cldm(self) -> None:
        self.cldm: ControlLDM = instantiate_from_config(
            OmegaConf.load('DiffBIR-main/configs/inference/cldm.yaml')
        )
        ...
        if self.args.version in ["v1", "v2"]:
            config = "DiffBIR-main/configs/inference/diffusion.yaml"
        else:
            config = "DiffBIR-main/configs/inference/diffusion_v2.1.yaml"
    ...
    def run(self) -> None:
        ...
        for lq in self.load_lq():
            ...
            #720, 1280x3 crop to 512x512
            gt = lq[200:712, (760+1280*2):(1272+1280*2)]
            lq = lq[200:712, 760:1272]
            ...
            for i in range(num_batches):
                ...
                if batch_samples.shape[0] == 1:
                    output = np.concatenate((lq, batch_samples[0],gt), axis = 1)
                    output = np.expand_dims(output, axis=0)
                    samples.extend(list(output))
                else:
