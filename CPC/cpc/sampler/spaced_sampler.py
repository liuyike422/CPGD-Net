from typing import Optional, Tuple, Dict, Literal, Optional
...
class SpacedSampler(Sampler):
    ...
    def apply_model(
        ...
        if uncond is None or cfg_scale == 1.0:
            model_output = model(x, model_t, cond, cp)
        else:
            model_cond = model(x, model_t, cond, cp)
            model_uncond = model(x, model_t, uncond, cp)
    ...
    def p_sample(
        ...
        # predict x_0
        model_output = self.apply_model(model, x, model_t, cond, uncond, cp, cfg_scale)
    ...
    def sample(
        ...
        tile_size: int = -1,
        tile_stride: int = -1,
        x_T: Optional[torch.Tensor] = None, #torch.Tensor | None = None,
        progress: bool = True,
        dict_input = None,
    ) -> torch.Tensor:
        ...
        if tiled:
            ...
            model.forward = make_tiled_fn(
                lambda x_tile, t, cond, cp, hi, hi_end, wi, wi_end: (
                    forward(
                        x_tile,
                        t,
                        {
